...
class VideoBaseModel(SRModel):
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):
            ...
            if not hasattr(self, 'metric_results'):  # only execute in the first run
                ...
                for folder, num_frame in num_frame_each_folder.items():
                    self.metric_results[folder] = torch.zeros(
                        num_frame, len(self.opt['val']['metrics']), dtype=torch.float32, device='cuda' if torch.cuda.is_available() else 'cpu')
        ...
        for idx in range(rank, len(dataset), world_size):
            ...
            crop_length = self.opt['val'].get('crop_length', 0)
            if crop_length > 0:
                b, t, c, h, w = val_data['lq'].shape
                _, c2, h2, w2 = val_data['lq'].shape
                sh, sw = int(h2/h), int(w2/w)

                visuals = torch.zeros(b, c2, h2, w2)
                for start_h in range(0, h, crop_length):
                    end_h = min(h, start_h+crop_length)
                    for start_w in range(0, w, crop_length):
                        end_w = min(h, start_h+crop_length)
                        
                        _val_data = {'lq':val_data['lq'][:, :, :, start_h:end_h, start_w:end_w]}
                        self.feed_data(_val_data)
                        self.test()
                        visuals[:, :, start_h*sh:end_h*sh, start_w*sw:end_w*sw] = self.get_current_visuals()
